...
public class CategoryService(ApplicationDbContext dbContext) : ICategoryService
{
    ...
    {
        if (await dbContext.Categories.FirstOrDefaultAsync(c => c.Name == name) != null)
    ...
    private async Task<Category> GetParentCategory(string parentName) =>
        await dbContext.Categories.FirstOrDefaultAsync(c => c.Name == parentName) ??
        throw new ArgumentException($"There is no category with parent name '{parentName}'!");
    ...
    public async Task<Category?> Add(string name, string? parentName)
    {
        ...
        await dbContext.Categories.AddAsync(Category.CreateInstance(await CheckName(name), parentCategory));
    ...
    {
        await BaseChange(oldName, category => category.Name = CheckName(newName).Result);
